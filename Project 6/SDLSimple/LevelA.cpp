...
#define LEVELA_WIDTH 25
#define LEVELA_HEIGHT 15
...
void LevelA::initialise()
{
    ...
    m_game_state.map = new Map(LEVELA_WIDTH, LEVELA_HEIGHT, LEVEL_DATA, map_texture_id, 0.5f, 9, 11);
    ...
    {
    m_game_state.enemies[i] =  Entity(enemy_texture_id, 0.0f, acceleration, 0.0f, enemy_walking_animation, 0.0f, 4, 0, 4, 4, 0.1f, 0.1f, ENEMY);
    ...
    m_game_state.enemies[0].set_speed(0.5f);
...
void LevelA::update(float delta_time)
{
    m_game_state.player->update(delta_time, m_game_state.player, m_game_state.enemies, ENEMY_COUNT, m_game_state.map);
        ...
        if (player_position.x > LEVELA_WIDTH * 0.5f) player_position.x = LEVELA_WIDTH * 0.5f;
        if (player_position.y > -0.5f) player_position.y = -0.5f;
        if (player_position.y < -LEVELA_HEIGHT * 0.5f) player_position.y = -LEVELA_HEIGHT * 0.5f;
        ...
        if (camera_x > (LEVELA_WIDTH * 0.5f) - 5.0f) camera_x = (LEVELA_WIDTH * 0.5f) - 5.0f;
        if (camera_y < -(LEVELA_HEIGHT * 0.5f) + 3.5f) camera_y = -(LEVELA_HEIGHT * 0.5f) + 3.5f;
...
//                m_game_state.next_scene_id = -1; // Use -1 or a specific ID for the game over screen
    ...
    for (int i = 0; i < COINS_COUNT; i++) {
        ...
        if (m_coins_collected == COINS_COUNT) {
            ...
            LEVEL_DATA[7 * LEVELA_WIDTH + 22] = 50;
